...
- **THEN** it exposes `/v1/models`, `/v1/chat/completions`, `/v1/embeddings`, and `/health` with behavior compatible with the current service contract, implemented via async axum handlers

#### Scenario: CLI parity
- **WHEN** an operator invokes `run`, `auth`, `status`, `config`, `models`, `refresh`, `migrate-config`, or `version` against the Rust runtime
- **THEN** every command SHALL produce behavior equivalent to the Go implementation, including `auth copilot` and `auth codex` flows which SHALL be fully functional (not stub)
...
- **THEN** the runtime preserves the current special handling that ignores the client model and applies the service's Copilot model-selection behavior, implemented via the async Provider trait

#### Scenario: Codex auth-mode parity
- **WHEN** Codex runs in API-key mode or ChatGPT/device-auth mode under the Rust runtime
- **THEN** the runtime preserves the current credential-source and auth-isolation behavior for each mode as defined in the rust-provider-runtime spec
